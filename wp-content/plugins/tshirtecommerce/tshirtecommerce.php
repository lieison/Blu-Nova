...
function setPermission($file, $mode = 0755)
{
	if (is_array($file))
	{
		foreach($file as $f)
		{
			$path = get_home_path() .'tshirtecommerce/'.$f;
			if (file_exists($path) !== false)
			{
				chmod($path, $mode);
			}
	...
	{
		$path = get_home_path() .'tshirtecommerce/'.$file;
		if (file_exists($path) !== false)
		{
			chmod($path, $mode);
		}
...
function designer_manage()
{
	...
	if ($check == true)
	{
		$url = site_url('tshirtecommerce/admin/index.php');		
	...
	else
	{
		...
		echo 'Sorry, your server not support unzipping the file. Please click <a href="'.$download.'"><strong>here</strong></a> to download file, unzip and upload to path '.get_home_path();
	}		
}
...
function online_designer_update()
{
	...
	if (!empty($_GET['action']))
	{
		...
		if ($data != false)
		{		
			$path = get_home_path() . $version;
			if(file_put_contents($path, $data))
			{
				WP_Filesystem();
				$unzipfile = unzip_file( $path, get_home_path());
				if ( $unzipfile ) {
	...
	if ($link != '')
	{
		...
		if (isset($price[0]) && $price[0] > 0)
		{
			...
			if (count($array) > 1)
			{
				...
				echo '<a class="button" href="'.$link.'">'.$opt_val['btn-custom'].'</a>';			
			}
			else
			{
				$link = add_query_arg( array('product_id'=>$product_id), $page );
				echo '<a class="button" href="'.$link.'">'.$opt_val['btn-start'].'</a>';	
			}
			echo '</div><br />';
		}
		else
		{
			...
			elseif( $product->is_type( 'variable' ) )
			{
				...
				add_action( 'woocommerce_before_single_variation', 'design_button_variable', 10, 0);
			}
			
		}		
	}
}
add_action( 'woocommerce_after_shop_loop_item_title', 'design_button' );
...
function design_button_variable()
{
	...
	$html .= '<a class="button" onclick="variationProduct(this)" href="javascript:void(0);">Start Design</a>';						
...
function theme_name_scripts() {	 
	wp_enqueue_script( 'app', plugins_url( 'assets/js/app.js', __FILE__ ), array(), '1.0.0', true );		
}
...
function wp_ajax_woo_products()
{
	...
	$json = get_home_path() .'tshirtecommerce/data/products.json';
	$array = array(
		'products' => array()
	);
	if (file_exists($json))
		...
		if ($string != false)
		{
			...
			if ( isset($products->products) && count($products->products) > 0)
			{
				...
				if (isset($_POST['id']) && $_POST['id'] > 0)
				{
					...
					$cate_file = get_home_path() .'tshirtecommerce/data/product_categories.json';				
					if (file_exists($cate_file))
					{
						$product_ids = array();
						$content = openURL($cate_file);
						if ($content != false)
						{
							$data = json_decode($content);
				...
				foreach($products->products as $product)
				{
					if ( in_array($product->id, $design_ids) && in_array($product->id, $cate_products) )
					{
						...
						{
							$product->attribute = getAttributes($product->attributes);
						}
						else
						{
							$product->attribute = '';
						}
						$product->attribute .= quantity($product->min_order);
...
function tshirtecommerce_func( $atts )
{
	...
	if ($product_id == 0)
	{
		$html .= '<div class="alert alert-danger" role="alert">Please add product and choose design in woocommerce.</div>';
	}
	else
	{
		...
		$html .= '<script type="text/javascript">var woo_url_cart = "'.$cart_url.'"; var wp_ajaxurl = "'.admin_url('admin-ajax.php').'"; var urlDesign = "'.$page.'";</script>';
		$url = '';
