 ...
 */
	...
	if ( is_array( $mega_main_menu->get_option( 'coercive_styles' ) ) && in_array( 'true', $mega_main_menu->get_option( 'coercive_styles', array() ) ) ) {
		remove_all_filters( 'wp_nav_menu_items', 1900 ); 
		remove_all_filters( 'wp_nav_menu_args', 1900 ); 
	}
	add_filter( 'wp_nav_menu_items', 'mmm_nav_search', 2000, 8 );
	add_filter( 'wp_nav_menu_items', 'mmm_nav_woo_cart', 2010, 8 );
	add_filter( 'wp_nav_menu_items', 'mmm_nav_buddypress', 2020, 8 );
	add_filter( 'wp_nav_menu_items', 'mmm_nav_wpml_switcher', 2030, 8 );
	...
	function mmm_set_walkers ( $args ){
		...
		if ( $indefinite_location_mode === true && isset( $args['theme_location'] ) && $args['theme_location'] == '' && isset( $mega_menu_locations[ 1 ] ) ) {
			$args['theme_location'] = $mega_menu_locations[ $mega_menu_location_id ];
			$theme_location_safe_name = $mega_menu_locations[ $mega_menu_location_id ];
			$container_class[] = $mega_menu_locations[ $mega_menu_location_id ];
		}

		if ( ( is_array( $mega_menu_locations ) && in_array( $theme_location_safe_name, $mega_menu_locations ) ) || ( $indefinite_location_mode === true ) || ( $args['theme_location'] == 'mega_main_sidebar_menu' ) || ( $args['theme_location'] == 'mega_main_flyout_menu' ) ) {
			...
			$container_class[] = 'pushing_content-' . ( ( is_array( $mega_main_menu->get_option( $theme_location_safe_name . '_pushing_content' ) ) && in_array( 'true', $mega_main_menu->get_option( $theme_location_safe_name . '_pushing_content' ) ) ) ? 'enable' : 'disable' );
			...
			$container_class[] = 'dropdowns_trigger-' . $mega_main_menu->get_option( $theme_location_safe_name . '_dropdowns_trigger', 'hover' ) ;
			...
			if ( has_filter( 'mmm_container_class' ) ) {
				$container_class_imploded .= ' ' . esc_attr( apply_filters( 'mmm_container_class', $container_class, $container_class ) );
			}
			...
			if( ( is_array( $mega_main_menu->get_option( $theme_location_safe_name . '_included_components' ) ) && in_array( 'company_logo', $mega_main_menu->get_option( $theme_location_safe_name . '_included_components' ) ) ) && $mega_main_menu->get_option( 'logo_src' ) ) {
					...
					$items_wrap .= mm_common::ntab(5) . '<img src="' . str_replace( array('%','$'), array('',''), $mega_main_menu->get_option( 'logo_src' ) ) . '" alt="' . get_bloginfo( 'name' ) . '" />';
			...
			$items_wrap .= mm_common::ntab(6) . $mega_main_menu->get_option( $theme_location_safe_name . '_mobile_label', 'Menu' ) . ' &nbsp;';
	...
	function mmm_nav_search( $items, $args ) {
		...
		if( isset( $args->theme_location ) ) {
			...
			if( ( in_array( $args->theme_location, $mega_menu_locations) ) && is_array( $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) && in_array( 'search_box', $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) ) {
				...
				$icon = 'custom_icon_search';
				if ( in_array( 'icomoon', $icon_sets ) ) {
					$icon = 'im-icon-search-3';
				...
				$searchform .= mm_common::ntab(2) . '<form method="get" id="mega_main_menu_searchform" action="' . esc_url( home_url( '/' ) ) . '">';
				$searchform .= mm_common::ntab(3) . '<i class="' . $icon . ' icosearch"></i>';
				$searchform .= mm_common::ntab(3) . '<input type="submit" class="submit" name="submit" id="searchsubmit" value="' . esc_attr__( 'Search', 'searchform_textdomain' ) . '" />';
				$searchform .= mm_common::ntab(3) . '<input type="text" class="field" name="s" id="s" />';
				$searchform .= mm_common::ntab(2) . '</form>';
				...
				if ( class_exists( 'BuddyPress' ) ){
					...
					if ( is_user_logged_in() ) {
						...
						foreach ( $bp->bp_nav as $key => $component ) {
							...
							$buddypress_item .= mm_common::ntab(3) . '<li class="menu-item default_dropdown ' . $drop_side . '">';
							...
							if ( is_array( $bp->bp_options_nav[ $component[ 'slug' ] ] ) ) {
								...
								foreach ( $bp->bp_options_nav[ $component[ 'slug' ] ] as $key => $sub_component ) {
									$buddypress_item .= mm_common::ntab(5) . '<li class="menu-item default_dropdown ' . $drop_side . '">';
						...
						$buddypress_item .= mm_common::ntab(2) . '<ul class="mega_dropdown">';
						ob_start();
							the_widget( 'BP_Core_Login_Widget', array( 'title' => '' ), false );
							$form .= ob_get_contents();
						ob_end_clean();
						$buddypress_item .= mm_common::ntab(3) . $form;
						$buddypress_item .= mm_common::ntab(2) . '</ul><!-- class="mega_dropdown" -->';
						$buddypress_item .= mm_common::ntab(1) . '</li><!-- class="nav_buddypress" -->' . mm_common::ntab(0);
					}
