...
<form id="adminForm" method="post" name="adminForm" action="<?php echo site_url_ci('index.php/product'); ?>">
<div class="row">
	<div class="col-md-6">
		<div class="row">
			<div class="col-sm-3">
	...
	<div class="col-md-6">
		<p class="pull-right">
			<a href="<?php echo site_url_ci('index.php/product/edit'); ?>" title="<?php lang('add'); ?>" class="btn btn-primary">
	...
	<table class="table table-bordered table-hover" id="sample-table-1">
		...
		?>
			...
			<tr>
				...
				<td>
					<a href="<?php echo site_url_ci('index.php/product/edit/' . $product->id); ?>" title=""><?php echo $product->title; ?></a>
				</td>
				<td class="center">
				...
				<td class="center">
					<?php if ($product->published == 1){ ?>
						<a href="<?php echo site_url_ci('index.php/product/unpublish/' . $product->id); ?>" class="btn btn-success btn-xs"><?php lang('publish'); ?></a>
					<?php }else{ ?>
						<a href="<?php echo site_url_ci('index.php/product/publish/' . $product->id); ?>" class="btn btn-bricky btn-xs"><?php lang('unpublish'); ?></a>
					<?php } ?>
				</td>
				<td class="center">
					<div class="btn-group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="<?php echo site_url_ci('index.php/product/edit/' . $product->id); ?>"><i class="fa fa-edit"></i> <?php lang('edit'); ?></a></li>
							<li><a href="<?php echo site_url_ci('index.php/product/copy/' . $product->id); ?>"><i class="fa fa-copy"></i> <?php lang('copy'); ?></a></li>
							<li><a href="<?php echo site_url_ci('index.php/product/delete/' . $product->id); ?>" onclick="return confirm('<?php lang('confirm_delete'); ?>')"><i class="glyphicon glyphicon-trash"></i> <?php lang('remove'); ?></a></li>
						</ul>
					</div>
				</td>
				<td class="center">
...
<script type="text/javascript">
	...
	function pagination(segment)
	{
		jQuery.ajax({
			type: "POST",
			url: '<?php echo site_url_ci('index.php/product/page/'); ?>'+segment,
			...
			beforeSend: function(){
				jQuery('#adminForm').block({
					...
					message: '<img src="<?php echo site_url_ci().'assets/images/loading.gif'?>" /> <?php lang('loading') ?>',
					css: {
						border: 'none',
						color: '#333',
						background: 'none'
	...
	{
		jQuery('#adminForm').attr('action', '<?php echo site_url_ci('index.php/product/copy'); ?>');
		return true;
	}
	...
	function delete_products()
	{
		...
		if(cf)
		{
			jQuery('#adminForm').attr('action', '<?php echo site_url_ci('index.php/product/delete'); ?>');
			return true;
		}else
		{
			return false;
