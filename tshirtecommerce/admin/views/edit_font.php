...
?>
<script src="<?php echo site_url_ci('assets/js/validate.js'); ?>"></script>
...
<form action="<?php echo site_url_ci() . 'index.php/settings/editfont/'.$data['id']; ?>" method="POST" class="fr-edit-fonts form-horizontal" id="panel-form" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<?php 
								if($data['font']['thumb'] != '')
								{
									echo '<img style="height: 20px;" src="'.str_replace('admin/', '', site_url_ci()).'data/fonts/'.$data['font']['thumb'].'" alt="">';
								}
							?>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="col-md-10">
							<div class="pull-right">
								...
								<a class="btn modal-close btn-danger" href="<?php echo site_url_ci() ?>index.php/settings/fonts"><?php echo lang('close'); ?></a>
							</div>
						</div>
					</div>
				</div>				
...
<div class="modal fade" id="modal_edit_cate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<form action="<?php echo site_url_ci().'index.php/settings/catefont'?>" method="post" id="fr-edit-cate">
...
<div style="display: none;">
	<form action="<?php echo site_url_ci().'index.php/settings/delcatefont'?>" method="post" id="fr-del-cate">
		<input id="del-cate-font" type="hidden" name="id" value=""/>
	</form>
</div>

<script type="text/javascript">
	var base_url 	= '<?php echo str_replace('admin/', '', site_url_ci()); ?>';
	var fonts 	= [];
	...
	function addCate() {
		...
		html = html + '<form method="post" action="<?php echo site_url_ci('index.php/settings/catefont')?>" id="form-add" class="form-add">';
	...
	function actionCate(type) 
	{
		if(type == 'edit')
		{
			...
			var url = '<?php echo site_url_ci('index.php/settings/catefont'); ?>';
		}else if(type == 'add')
		{
		...
		}else
		{
			...
			var url = '<?php echo site_url_ci('index.php/settings/delcatefont'); ?>';
		}
