...
var dagFiles = {
	...
	file: {			
		edit: function(path, e){
			...
			if(title != '' && title != null)
			{
				var url = base + 'index.php/media/filefename';				
		...
		remove: function(path, e){
			...
			var url = base + 'index.php/media/fileremove';
	...
	ajax:{
		folder: function(e){	
			...
			var url = base + 'index.php/media/folder';
	...
	folder:{
			...
			$d.post(url, { path: path }).done(function(content) {
				...
				if(data.files.length > 0)
				{
					for(i=0; i<data.files.length; i++)
					{
						...
						if(data.files[i].exten == 'png' || data.files[i].exten == 'gif' || data.files[i].exten == 'jpg')
						{
		...
		add: function(e, path){
			...
			else
			{
				...
				if ( title != null)
				{
					...
					var url = base + 'index.php/media/add';
		...
		rename: function(src, name, e){
			...
			if(typeof e == 'undefined' || e == false)
			{
				...
				if(title != null)
				{
					var url = base + 'index.php/media/rename';
		...
		remove: function(src, o, load){
			...
			{
				var check = confirm('You sure want remove this folder?');			
				if(check == false) return false;
			
				var url = base + 'index.php/media/remove';
